...
struct Graph{
    ...
    void intializenodes(){
        ...
        for(int i=0;i<n;i++){
            nodes[i].label=i+1;  //labling according to the given graph
        }
    }
    ...
    void addedge(int u, int v){
        ...
        for (int i=0;i<n;i++){
            if(nodes[i].label==u){  //finding node u
                nodes[i].neighbours.push_back(v);   //pushing node v into the neighbours list of u
            }
        }

        //select node v and push u into v's neighbour
        for (int i=0;i<n;i++){
            if(nodes[i].label==v){    //doing vice versa
