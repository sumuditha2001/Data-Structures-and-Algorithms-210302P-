...
struct HashTable{
    ...
    int hashfunc(string user_name){
        ...
        int hash = 0;
	//add your code below
        for(int i=0;i<user_name.length();i++){   //iterating over the string
            sum +=user_name[i];      //adding the ascii values of each character
        }
        hash = sum % MAX_LENGTH;
	...
	//add an if condition to complete the code here
        if (empty){
            password[hash]=user_password;   //if the slot is empty store the password
        }else{   //otherwise dont
            cout<<"hash collision occurs cant insert the value"<<endl;
        }
	...
	//add an if condition to complete the code here
        if (empty){
            ...
            cout<<"password is : "<<password[hash]<<endl;   //if the username exists display the password.
        }
